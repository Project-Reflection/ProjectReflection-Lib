 ...
 * <a href="https://github.com/DimensionalDevelopment/JustEnoughIDs/blob/master/src/main/java/org/dimdev/jeid/mixin/init/JEIDMixinLoader.java">...</a>
 */
@Mixin(Loader.class)
public class MixinLoader {
    ...
    @Redirect(method = "loadMods",at = @At(value = "INVOKE",
            target = "Lnet/minecraftforge/fml/common/LoadController;distributeStateMessage(Lnet/minecraftforge/fml/common/LoaderState;[Ljava/lang/Object;)V"),
            remap = false)
    private void redirect_distributeStateMessage(LoadController instance, LoaderState state, Object[] eventData) {
        ...
        if(state == LoaderState.CONSTRUCTING) {
            ...
            for (Map.Entry<String, List<Supplier<Boolean>>> entry : TwelvefoldRegistryAPI.getLateMixins().entrySet()) {
                ...
                for (Supplier<Boolean> supplier : entry.getValue()) {
                    ...
                    if (supplied == null)
                        TwelvefoldPlugin.LOGGER.warn("TwelvefoldBooter received null value for individual supplier from \"" + entry.getKey() + "\" for late mixin application.");
        ...
        try {
            ...
            instance.distributeStateMessage(state,eventData);
        }
    }
}